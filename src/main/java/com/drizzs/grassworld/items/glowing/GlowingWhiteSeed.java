...
public class GlowingWhiteSeed extends Item {

    public GlowingWhiteSeed(Properties group)
    {
    ...
    public ActionResultType onItemUse(ItemUseContext context) {
        ...
        if (!(item.getItem() == GrassWorldItems.glowingwhiteseed)){
        ...
        if (world.getBlockState(blockpos).getBlock() == Blocks.GRASS_BLOCK || world.getBlockState(blockpos).getBlock() == Blocks.DIRT)
        {
            ...
            world.setBlockState(blockpos, GrassWorldBlocks.glowingwhitegrass.getDefaultState());
            item.shrink(1);
        }
