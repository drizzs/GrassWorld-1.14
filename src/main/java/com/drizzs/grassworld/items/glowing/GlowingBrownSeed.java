...
public class GlowingBrownSeed extends Item {

    public GlowingBrownSeed(Properties group)
    {
    ...
    public ActionResultType onItemUse(ItemUseContext context) {
        ...
        if (!(item.getItem() == GrassWorldItems.glowingbrownseed)){
        ...
        if (world.getBlockState(blockpos).getBlock() == Blocks.GRASS_BLOCK || world.getBlockState(blockpos).getBlock() == Blocks.DIRT)
        {
            ...
            world.setBlockState(blockpos, GrassWorldBlocks.glowingbrowngrass.getDefaultState());
            item.shrink(1);
        }
