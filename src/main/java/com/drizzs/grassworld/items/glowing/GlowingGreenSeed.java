...
public class GlowingGreenSeed extends Item {

    public GlowingGreenSeed(Properties group)
    {
    ...
    public ActionResultType onItemUse(ItemUseContext context) {
        ...
        if (!(item.getItem() == GrassWorldItems.glowinggreenseed)){
        ...
        if (world.getBlockState(blockpos).getBlock() == Blocks.GRASS_BLOCK || world.getBlockState(blockpos).getBlock() == Blocks.DIRT)
        {
            ...
            world.setBlockState(blockpos, GrassWorldBlocks.glowinggreengrass.getDefaultState());
            item.shrink(1);
        }
