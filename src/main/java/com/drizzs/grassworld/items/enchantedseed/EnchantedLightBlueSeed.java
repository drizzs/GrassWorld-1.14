...
public class EnchantedLightBlueSeed extends Item {

    public EnchantedLightBlueSeed(Properties group)
    {
    ...
    public ActionResultType onItemUse(ItemUseContext context) {
        ...
        if (!(item.getItem() == GrassWorldItems.enchanted_lightblueseed)){
        ...
        if (world.getBlockState(blockpos).getBlock() == Blocks.GRASS_BLOCK || world.getBlockState(blockpos).getBlock() == Blocks.DIRT)
        {
            ...
            world.setBlockState(blockpos, GrassWorldBlocks.enchanted_lightblue_grass.getDefaultState());
            item.shrink(1);
        }
