...
public class EnchantedWhiteSeed extends Item {

    public EnchantedWhiteSeed(Properties group)
    {
    ...
    public ActionResultType onItemUse(ItemUseContext context) {
        ...
        if (!(item.getItem() == GrassWorldItems.enchanted_whiteseed)){
        ...
        if (world.getBlockState(blockpos).getBlock() == Blocks.GRASS_BLOCK || world.getBlockState(blockpos).getBlock() == Blocks.DIRT)
        {
            ...
            world.setBlockState(blockpos, GrassWorldBlocks.enchanted_white_grass.getDefaultState());
            item.shrink(1);
        }
