...
public class BlueIslandFeature extends Feature<NoFeatureConfig> {
    public BlueIslandFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> config) {
        super(config);
    }
                    ...
                    if ((float)(k * k + j * j) <= (f + 1.0F) * (f + 1.0F)) {
                        ...
                        if(world.getBlockState(pos1.up()).isIn(GrassTags.Blocks.GRASSWORLDGRASS)){
                            ...
                            if(next == 0 || next == 3 || next == 4 || next == 7){
                                this.setBlockState(world, pos1, GrassContentLib.blue_grass.getDefaultState());
                            }
                            else if(next == 1 || next == 5 || next == 8){
                                this.setBlockState(world, pos1, GrassContentLib.fancy_blue_grass.getDefaultState());
                            }
                            else if(next == 2){
                                this.setBlockState(world, pos1, GrassContentLib.enchanted_blue_grass.getDefaultState());
                            }
                            else if(next == 6){
                                this.setBlockState(world, pos1, GrassContentLib.glowingbluegrass.getDefaultState());
                            }
                        }
                    }
                }
