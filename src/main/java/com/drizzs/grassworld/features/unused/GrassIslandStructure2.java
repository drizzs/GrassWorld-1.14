...
public class GrassIslandStructure2 extends ScatteredStructure<NoFeatureConfig> {

    public GrassIslandStructure2(Function<Dynamic<?>, ? extends NoFeatureConfig> config) {
        super(config);
    }

    public boolean startStructure(IWorld world, BlockPos pos) {
        StructureStart start = this.getStart(world, pos, true);
        if (start != StructureStart.DUMMY && start instanceof GrassIslandStructure2.Start && !start.getComponents().isEmpty()) {
            StructurePiece piece = (StructurePiece)start.getComponents().get(0);
            return piece instanceof GrassIslandPiece2;
        } else {
            return false;
        }
    }
    ...
    public IStartFactory getStartFactory() {
        return GrassIslandStructure2.Start::new;
    }
    ...
    public static class Start extends StructureStart {
        ...
        public void init(ChunkGenerator<?> generator, TemplateManager templateManagerIn, int chunkX, int chunkZ, Biome biomeIn) {
            ...
            GrassIslandPiece2 piece = new GrassIslandPiece2(templateManagerIn, sizes[this.rand.nextInt(sizes.length)],new BlockPos(x, y, z));
